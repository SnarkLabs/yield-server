...
const getPositionsOfPool = async (poolId) => {
  ...
  while (true) {
    const queryString = gql`
    query {
      positions(first: 100, skip: ${i * 1000}, where: { liquidity_gt: 0, pool: "${poolId}" }) {
        id
        owner
        tickLower {
          tickIdx
...
const getPositionsInEternalFarming = async (farmingId) => {
  ...
  while (true) {
    const queryString = `{
      deposits(where:{eternalFarming:"${farmingId}"}, first:100, skip:${i * 1000}){
        id
      }
    }`;
...
const getPositionsById = async (tokenIds) => {
  ...
  while (true) {
    const queryString = `{
      positions(where:{id_in:${JSON.stringify(tokenIds)}}, first:100, skip:${i * 1000}){
        id
        liquidity
        tickLower{
          tickIdx
