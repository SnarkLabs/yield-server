...
const { queryPools, queryPrior, queryBlock } = require('./queries');
...
const topLvl = async (chainString, timestamp, url) => {
  ...
  // console.log('poolsFees', poolsFees, poolsCurrentTvl)

  const eternalAPRObj = await updateEternalFarmsApr();
  const poolsAPR = {};
  for (const pool of data) {
    const aprDataForPool = eternalAPRObj.farmPools[pool.id];
    const apr = aprDataForPool ? new BigNumber(aprDataForPool.rewardTokenApr) : new BigNumber(0);
  ...
  data = data.map((item) => {
    ...
    if (tvl > 30000) {
      ...
      return {
        ...
        tvlUsd: parseFloat(item.totalValueLockedUSD),
        apyBase: apr?.isNaN() ? 0 : apr?.toNumber(),
        underlyingTokens: [item.token0.id, item.token1.id],
        url: `https://app.stellaswap.com/pulsar/add/${item.token0.id}/${item.token1.id}`,
      };
    }
  });
